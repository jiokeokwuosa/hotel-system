...
<div class="row">
    <h3 class="center color">Verify Bar Order</h3>
    <?php  
    if(isset($_SESSION['c'])){
      $error=$_SESSION['c'];
   ...
   ?> 
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><?php echo ucwords(strtolower($db->getDrinkName($drink_id)));?></td>
            <td><?php echo $num;?></td>            
            <td> <?php echo date('d-m-y',strtotime($date_created));?></td>
            <td><?php echo"<a onclick='return checkApprove();' href=transact.php?key=$order_id&action=verifyBarOrder>";?><i class="fa fa-sign-out" style="color: red;"></i></a> </td>
        </tr> 
        <?php
