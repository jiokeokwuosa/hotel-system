...
class Database{
    ...
    public function listRooms($id=null){
        $selectUsersList='<option value="">Select Room</option>';
        
        if(!empty($id)){
          $result=$this->select('rooms',array('status'=>$id)); 
        }else{
           $result=$this->select('rooms');     
           }
        ...
        if($this->numRow()>0){
          ...
          $selectUsersList.="<option value=$room_id>".ucwords(strtolower($name))."</option>";
          }  
        }
        return $selectUsersList;        
    }
    ...
    public function listDrinks(){
        $selectUsersList='<option value="">Select Drink</option>';      
        
           $result=$this->select('drinks');      
        
        if($this->numRow()>0){
          while($row=mysqli_fetch_assoc($result)){            
            extract($row);
                        
          $selectUsersList.="<option value=$drink_id>".ucwords(strtolower($name))."</option>";
          }  
        }
        return $selectUsersList;        
    }
    ...
    public function listFoods(){
        $selectUsersList='<option value="">Select Food</option>';      
        
           $result=$this->select('foods');      
        
        if($this->numRow()>0){
          while($row=mysqli_fetch_assoc($result)){            
            extract($row);
                        
          $selectUsersList.="<option value=$food_id>".ucwords(strtolower($name))."</option>";
          }  
        }
        return $selectUsersList;        
    }
    
    public function getStatus($access_level){ 
    ...
    public function getRoomName($room_id){ 
        $result=$this->select('rooms',array('room_id'=>$room_id));  
        $row=mysqli_fetch_assoc($result);
        extract($row);
        return ucfirst($name);       
    ...
    public function getDrinkName($drink_id){ 
        $result=$this->select('drinks',array('drink_id'=>$drink_id));  
        $row=mysqli_fetch_assoc($result);
        extract($row);
        return ucfirst($name);       
    ...
    public function getFoodName($food_id){ 
        $result=$this->select('foods',array('food_id'=>$food_id));  
        $row=mysqli_fetch_assoc($result);
        extract($row);
        return ucfirst($name);       
