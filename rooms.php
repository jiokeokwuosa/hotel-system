...
<div class="row">
    <h3 class="center color">Room(s)</h3>
    <?php  
    if(isset($_SESSION['c'])){
      $error=$_SESSION['c'];
...
<div class="row">
   ...
   $db->connect();
   $result=$db->select('rooms',array(),array('room_id'=>'desc'));
   $numrow=$db->numRow();
   if($numrow==0){
   ...
   ?> 
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><?php  echo"<a onclick='return checkDelete();' href=transact.php?key=$room_id&action=deleteRoom>";?><i class="glyphicon glyphicon-remove" style="color: red;"></i></a> </td>
            <td><?php  echo"<a onclick='return checkEdit();' href=transact.php?key=$room_id&action=editRoom>";?><i class="glyphicon glyphicon-pencil" style="color: black;"></i></a> </td>          
        </tr> 
        <?php
